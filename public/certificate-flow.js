(() => {
  ...
  function certificateMarkup() {
    ...
    return `<article id="tw-certificate" class="tw-certificate">
      <div class="tw-cert-inner">
        ...
        <p class="tw-cert-copy">has demonstrated typing proficiency by completing a typing test on <b>WPM<span>Test</span></b><br/>and has achieved the following results:</p>
  ...
  async function shareCertificate(){
    ...
    try { if (navigator.share) await navigator.share({title:'My WPMTest Typing Certificate', text, url:location.origin}); else { await navigator.clipboard.writeText(text+' '+location.origin); alert('Certificate result copied to clipboard.'); } } catch(e){}
  }
